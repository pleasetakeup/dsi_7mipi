     ...
     if((buf[0] & 0x80) ||((buf[0]&0x0F)<6)){
      GT911_WR_Reg(0x814E, temp, 1);
           ...
           if(x > 800){
						...
						MyTouchData.xLow[j] = (uint8_t)x & 0xFF;
						MyTouchData.xHigh[j] = (uint8_t)(x >> 8) & 0xFF;
						MyTouchData.yLow[j] = (uint8_t)y & 0xFF;
						MyTouchData.yHigh[j] = (uint8_t)(y >> 8) & 0xFF;
